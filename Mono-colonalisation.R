...
bacteria  <-  readRDS("InputData/bacteria.RDS")
...
if (is.na(simulation_sequence$GAPSEQ[j])==T) {
  no_bacterium <- T
}
...
for (p in 1:replications) {
  ...
  if (no_bacterium == F) {
    # Computing bacterial abundances
    final <- input * myvolume
    finallumen <- final * lumen
    finalout <- final * out
    finalinn <- final * inn
    bac <- (bacteria[which(names(bacteria)==simulation_sequence$GAPSEQ[j])])[[1]]
...
saveRDS(simlist, file = paste("OutputData/",simulation_sequence$Sample[j],
                                ".RDS",sep = ""), compress = T)
warnings()
